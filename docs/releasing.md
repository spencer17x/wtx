...
Pull requests and branch pushes do not trigger an automated CI workflow. `v*`
tag pushes trigger the release workflow, which reruns the tests and validates
the GoReleaser configuration before publishing.
...
The dry run validates the candidate version, runs `npm test` and `npm run release:check`, checks `npm pack --dry-run`, and runs `npm run release:snapshot`. It does not publish anything.

## Shipping a Release

1. Run `npm test` and `npm run release:check`.
2. Optionally run `Release Dry Run` against the exact ref and candidate version you plan to release.
3. Create a tag in the form `vX.Y.Z` on the commit being released, typically the intended `main` tip.
4. Push the tag.
